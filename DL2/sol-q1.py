...
def gradient_descent(w0, w1, X, y): # 0, 0, [], []

    gradient0 = (-2/X.size) * np.sum(y - (w0 + w1*X)) # 실제값 - (예측값)
    print(gradient0)
...
def main():
    X = np.array([1,2,3,4]).reshape((-1,1)) # (4,) => (4 x 1)
    print(X.shape)
    y = np.array([3.1, 4.9, 7.2, 8.9]).reshape((-1,1)) # (4,) => (4 x 1)
    print(y.shape)
