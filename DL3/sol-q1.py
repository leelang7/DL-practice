...
def build_model1(input_shape): # (1, 5, 5, 1)
    model = tf.keras.layers.Conv2D(1, kernel_size=(3, 3), strides=(1, 1), padding="same",
                          activation="relu", 
...
def build_model2(input_shape):
    model = Sequential([tf.keras.layers.Conv2D(4, kernel_size=(3, 3), strides=(1, 1), padding="same", input_shape=input_shape[1:]),
...
def main():
    input_shape = (1, 5, 5, 1) # 5x5, 흑백이미지, 1장(데이터셋 하나), B/H/W/C
