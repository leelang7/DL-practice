...
def load_data(num_words, max_len):
    ...
    X_train = pad_sequences(X_train, maxlen=max_len) # 130
    X_test = pad_sequences(X_test, maxlen=max_len)
    
    return X_train, X_test, y_train, y_test

def build_rnn_model(num_words, embedding_len): # 6000x100
    model = Sequential()
    
    # TODO: [지시사항 1번] 지시사항에 따라 모델을 완성하세요.
    model.add(layers.Embedding(num_words, embedding_len)) # 6000x100
...
def main(model=None, epochs=5):
    ...
    # IMDb 데이터셋을 불러옵니다.
    X_train, X_test, y_train, y_test = load_data(num_words, max_len) # 6000, 130
    print(X_train[0], y_train[0])
    
    if model is None:
        model = build_rnn_model(num_words, embedding_len) # 6000, 100
