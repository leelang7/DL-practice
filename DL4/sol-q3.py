...
def load_data(window_size):
    ...
    # 데이터셋을 구성합니다.
    X, y = [], []
    for i in range(len(raw_passengers) - window_size): # 144-4 = 140
        cur_passenger = raw_passengers[i:i + window_size] # 0:4
        target = raw_passengers[i + window_size] # 4
...
def build_rnn_model(window_size):
    ...
    model.add(layers.SimpleRNN(4, input_shape=(window_size, 1))) # 4x1
